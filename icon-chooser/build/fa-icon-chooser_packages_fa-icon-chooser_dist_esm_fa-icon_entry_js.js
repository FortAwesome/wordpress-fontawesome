"use strict";
(globalThis["webpackChunkicon_chooser"] = globalThis["webpackChunkicon_chooser"] || []).push([["fa-icon-chooser_packages_fa-icon-chooser_dist_esm_fa-icon_entry_js"],{
...
/* harmony export */   fa_icon: () => (/* binding */ FaIcon)
/* harmony export */ });
...
/* harmony import */ var _utils_cb65161a_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./utils-cb65161a.js */ "../../fa-icon-chooser/packages/fa-icon-chooser/dist/esm/utils-cb65161a.js");


const faIconCss = "";
let FaIcon = class {
  constructor(hostRef) {
    (0,_index_9d40404c_js__WEBPACK_IMPORTED_MODULE_0__.r)(this, hostRef);
    this.pro = false;
    this.loading = false;
  }
  ...
  componentWillLoad() {
    ...
    if (!this.svgApi) {
      console.error(`${_utils_cb65161a_js__WEBPACK_IMPORTED_MODULE_1__.C}: fa-icon: svgApi prop is needed but is missing`, this);
      return;
    }
    if (!(this.stylePrefix && this.name)) {
      console.error(`${_utils_cb65161a_js__WEBPACK_IMPORTED_MODULE_1__.C}: fa-icon: the 'stylePrefix' and 'name' props are needed to render this icon but not provided.`, this);
      return;
    }
    if (!this.familyStylePathSegment) {
      console.error(`${_utils_cb65161a_js__WEBPACK_IMPORTED_MODULE_1__.C}: fa-icon: the 'familyStylePathSegment' prop is required to render this icon but not provided.`, this);
      return;
    }
    const {
      findIconDefinition
    ...
    if (!this.pro) {
      console.error(`${_utils_cb65161a_js__WEBPACK_IMPORTED_MODULE_1__.C}: fa-icon: 'pro' prop is false but no free icon is available`, this);
      return;
    }
    if (!this.svgFetchBaseUrl) {
      console.error(`${_utils_cb65161a_js__WEBPACK_IMPORTED_MODULE_1__.C}: fa-icon: 'svgFetchBaseUrl' prop is absent but is necessary for fetching icon`, this);
      return;
    }
    if (!this.kitToken) {
      console.error(`${_utils_cb65161a_js__WEBPACK_IMPORTED_MODULE_1__.C}: fa-icon: 'kitToken' prop is absent but is necessary for accessing icon`, this);
      return;
    }
    this.loading = true;
    const iconUrl = `${this.svgFetchBaseUrl}/${this.familyStylePathSegment}/${this.name}.svg?token=${this.kitToken}`;
    const library = _utils_cb65161a_js__WEBPACK_IMPORTED_MODULE_1__.l.get(this, 'svgApi.library');
    if ('function' !== typeof this.getUrlText) {
      console.error(`${_utils_cb65161a_js__WEBPACK_IMPORTED_MODULE_1__.C}: fa-icon: 'getUrlText' prop is absent but is necessary for fetching icon`, this);
      return;
    }
    this.getUrlText(iconUrl).then(svg => {
      const iconDefinition = {
        iconName: this.name,
        prefix: this.stylePrefix,
        icon: (0,_utils_cb65161a_js__WEBPACK_IMPORTED_MODULE_1__.p)(svg)
      };
      library && library.add(iconDefinition);
      this.setIconDefinition(Object.assign({}, iconDefinition));
    }).catch(e => {
      console.error(`${_utils_cb65161a_js__WEBPACK_IMPORTED_MODULE_1__.C}: fa-icon: failed when using 'getUrlText' to fetch icon`, e, this);
    }).finally(() => {
      this.loading = false;
    });
  }
  buildSvg(iconDefinition, extraClasses) {
    ...
    const [width, height,,, svgPathData] = _utils_cb65161a_js__WEBPACK_IMPORTED_MODULE_1__.l.get(iconDefinition, 'icon', []);
    ...
    if (Array.isArray(svgPathData)) {
      return (0,_index_9d40404c_js__WEBPACK_IMPORTED_MODULE_0__.h)("svg", {
        class: allClasses,
        xmlns: "http://www.w3.org/2000/svg",
        viewBox: `0 0 ${width} ${height}`
      }, (0,_index_9d40404c_js__WEBPACK_IMPORTED_MODULE_0__.h)("path", {
        fill: "currentColor",
        class: "fa-primary",
        d: svgPathData[1]
      }), (0,_index_9d40404c_js__WEBPACK_IMPORTED_MODULE_0__.h)("path", {
  ...
  render() {
    return this.iconDefinition ? this.buildSvg(this.iconDefinition) : (0,_index_9d40404c_js__WEBPACK_IMPORTED_MODULE_0__.h)(_index_9d40404c_js__WEBPACK_IMPORTED_MODULE_0__.H, null);
  }
};
...
/***/ "../../fa-icon-chooser/packages/fa-icon-chooser/dist/esm/utils-cb65161a.js":
/*!*********************************************************************************!*\
  !*** ../../fa-icon-chooser/packages/fa-icon-chooser/dist/esm/utils-cb65161a.js ***!
  \*********************************************************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   C: () => (/* binding */ CONSOLE_MESSAGE_PREFIX),
/* harmony export */   F: () => (/* binding */ Fragment),
/* harmony export */   a: () => (/* binding */ buildIconChooserResult),
/* harmony export */   b: () => (/* binding */ buildDefaultIconsSearchResult),
/* harmony export */   c: () => (/* binding */ createFontAwesomeScriptElement),
/* harmony export */   f: () => (/* binding */ freeCdnBaseUrl),
/* harmony export */   i: () => (/* binding */ isValidSemver),
/* harmony export */   k: () => (/* binding */ kitAssetsBaseUrl),
/* harmony export */   l: () => (/* binding */ lodash),
/* harmony export */   p: () => (/* binding */ parseSvgText)
/* harmony export */ });
...
re[t.COERCERTL].lastIndex=-1;}if(match===null){return null;}return parse(match[2]+'.'+(match[3]||'0')+'.'+(match[4]||'0'),options);}});const FREE_CDN_URL='https://use.fontawesome.com';const PRO_KIT_ASSET_URL='https://ka-p.fontawesome.com';const FREE_KIT_ASSET_URL='https://ka-f.fontawesome.com';// Given a set of familyStyles, replace the term "ALL" in the defaultIconsSearchResult
...
function buildDefaultIconsSearchResult(familyStyles){const allNonBrandsFamilyStyles=[];for(const family in familyStyles){for(const style in familyStyles[family]){if('brands'!==style&&'brands'!==family&&'custom'!==style){allNonBrandsFamilyStyles.push({family,style});}}}const defaultIconsSearchResult=lodash.cloneDeep(defaultIconsSearchResultTemplate);const icons=lodash.get(defaultIconsSearchResult,'data.search',[]);for(const i of icons){if('ALL'===lodash.get(i,'familyStylesByLicense.pro')){lodash.set(i,'familyStylesByLicense.pro',allNonBrandsFamilyStyles);}}return defaultIconsSearchResult;}const viewBoxRe=/viewBox="0 0 ([0-9]+) ([0-9]+)"/;const singlePathRe=/path d="([^"]+)"/;const duotonePathRe=[/path d="(?<d1>[^"]+)".*path d="(?<d2>[^"]+)"/,/path class="(?<cls1>[^"]+)".*d="(?<d1>[^"]+)".*path class="(?<cls2>[^"]+)".*d="(?<d2>[^"]+)"/,/path class="(?<cls1>[^"]+)".*d="(?<d1>[^"]+)"/];const CONSOLE_MESSAGE_PREFIX='Font Awesome Icon Chooser';function parseSvgText(svgText){let val=null;let path=null;const viewBox=svgText.match(viewBoxRe);const singlePath=svgText.match(singlePathRe);const duotonePath=svgText.match(duotonePathRe[0])||svgText.match(duotonePathRe[1])||svgText.match(duotonePathRe[2]);if(duotonePath){const{cls1,d1,cls2,d2}=duotonePath.groups;if(d1&&d2&&!cls1&&!cls2){path=[d1,d2];}else if(d1&&cls1&&!d2){path=cls1.indexOf('primary')>-1?['',d1]:[d1,''];}else if(d1&&d2&&cls1&&cls2){path=cls1.indexOf('primary')>-1?[d2,d1]:[d1,d2];}}else if(singlePath&&singlePath.length===2){path=singlePath[1];}if(viewBox&&path){val=[parseInt(viewBox[1],10),parseInt(viewBox[2],10),[],null,path];}return val;}function freeCdnBaseUrl(){return FREE_CDN_URL;}function kitAssetsBaseUrl(pro){return pro?PRO_KIT_ASSET_URL:FREE_KIT_ASSET_URL;}async function createFontAwesomeScriptElement(getUrlText,pro,version,baseUrl,kitToken){const license=pro?'pro':'free';const assetUrl=kitToken?`${baseUrl}/releases/v${version}/js/${license}.min.js?token=${kitToken}`:`${baseUrl}/releases/v${version}/js/all.js`;try{if('function'!==typeof getUrlText){throw new Error("Font Awesome Icon Chooser: expected getUrlText to be a function but it wasn't");}const scriptContent=await getUrlText(assetUrl);const script=document.createElement('SCRIPT');const text=document.createTextNode(scriptContent);script.appendChild(text);script.setAttribute('data-auto-replace-svg','false');script.setAttribute('data-auto-add-css','false');script.setAttribute('type','text/javascript');return script;}catch(e){console.error(e);throw new Error(e);}}function buildIconChooserResult(iconDefinition){return iconDefinition;}function isValidSemver(val){return!!semver.valid(val);}const Fragment=(_props,children)=>[...children];

/***/ })

}]);
//# sourceMappingURL=fa-icon-chooser_packages_fa-icon-chooser_dist_esm_fa-icon_entry_js.js.map